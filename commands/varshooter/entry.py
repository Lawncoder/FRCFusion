...
CMD_ID = f'{config.COMPANY_NAME}_{config.ADDIN_NAME}_Variable'
CMD_NAME = 'Variable Hood Shooter'
CMD_Description = 'Creates a Moving hood shooter'
...
def command_created(args: adsk.core.CommandCreatedEventArgs):
    ...
    try: 
        ...
        inputs.addAngleValueCommandInput('min_angle', 'Minimum Angle', adsk.core.ValueInput.createByReal(math.radians(15)))
        inputs.addAngleValueCommandInput('max_angle', 'Maximum Angle', adsk.core.ValueInput.createByReal(math.radians(60)))
        inputs.addValueInput('roller_diameter', "Hood Roller Diameter", defaultLengthUnits, adsk.core.ValueInput.createByString('2 in'))
        inputs.addIntegerSliderListCommandInput('count', "Hood Roller Count", [0,3,4,5])
...
def command_execute(args: adsk.core.CommandEventArgs):
    ...
    try:
        ...
        min_angle_input = inputs.itemById('min_angle')
        min_angle = min_angle_input.value  # radians
        min_angle_deg = math.degrees(min_angle)

        max_angle_input = inputs.itemById('max_angle')
        max_angle = max_angle_input.value  # radians
        ...
        occ = workingComp.occurrences.addNewComponent(adsk.core.Matrix3D.create())
        comp = occ.component
        ...
        offset_inches = 0.5 if roller_count == 0 else roller_diameter/2.54
        ...
        arc = plate_sketch.sketchCurves.sketchArcs.addByCenterStartSweep(futil.pZero, futil.pointFromOffset(futil.pZero, -(diameter_inches/2 + ball_diameter_inches - compression_inches + 0.5 + offset_inches),0 ), -min_angle)
        ...
        startPoint = futil.pointFromOffset(futil.pZero, -(diameter_inches/2 + ball_diameter_inches - compression_inches + 0.5 + offset_inches), 0)
    
        arc1 = middle_sketch.sketchCurves.sketchArcs.addByCenterStartSweep(futil.pZero, futil.pointFromOffset(futil.pZero, -(diameter_inches/2 + ball_diameter_inches - compression_inches + 0.5 + offset_inches),0 ), - min_angle)
        arc2 = middle_sketch.sketchCurves.sketchArcs.addByCenterStartSweep(futil.pZero, futil.pointFromOffset(futil.pZero, -(diameter_inches/2 + ball_diameter_inches - compression_inches + offset_inches),0 ), -min_angle)
        ...
        second_occ = workingComp.occurrences.addNewComponent(adsk.core.Matrix3D.create())
        second_comp = second_occ.component

        plate_sketch = second_comp.sketches.add(second_comp.xYConstructionPlane)

        pivotCircle = plate_sketch.sketchCurves.sketchCircles.addByCenterRadius(futil.pZero, diameter/2)
        pivotPoint = pivotCircle.centerSketchPoint
        
        arc1 = plate_sketch.sketchCurves.sketchArcs.addByCenterStartSweep(futil.pZero, futil.pointFromOffset(futil.pZero, -(diameter_inches/2 + ball_diameter_inches - compression_inches + offset_inches), 0), (min_angle-max_angle))
       
        plate_sketch.sketchCurves.sketchLines.addByTwoPoints(arc1.startSketchPoint, futil.pZero)
        plate_sketch.sketchCurves.sketchLines.addByTwoPoints(arc1.endSketchPoint, futil.pZero)
        ...
        feature = second_comp.features.extrudeFeatures.addSimple(collection, adsk.core.ValueInput.createByReal(2.54 * 0.0625), adsk.fusion.FeatureOperations.NewBodyFeatureOperation)
        featureCollection.clear()
        featureCollection.add(feature)
        plane = second_comp.constructionPlanes.createInput()
        plane.setByOffset(second_comp.xYConstructionPlane, adsk.core.ValueInput.createByReal(width/2))
        realPlane = second_comp.constructionPlanes.add(plane)
        mirrorInput = second_comp.features.mirrorFeatures.createInput(featureCollection, realPlane)
        second_comp.features.mirrorFeatures.add(mirrorInput)

        middle_sketch = second_comp.sketches.add(second_comp.xYConstructionPlane)
        middle_sketch.sketchCurves.sketchCircles.addByCenterRadius(futil.pZero, diameter/2)
        if roller_count == 0:
            arcOne = middle_sketch.sketchCurves.sketchArcs.addByCenterStartSweep(futil.pZero, futil.pointFromOffset(futil.pZero, -(diameter_inches/2 + ball_diameter_inches - compression_inches + offset_inches),0 ), (min_angle-max_angle))
          
            arcTwo = middle_sketch.sketchCurves.sketchArcs.addByCenterStartSweep(futil.pZero, futil.pointFromOffset(futil.pZero, -(diameter_inches/2 + ball_diameter_inches - compression_inches - 0.5 + offset_inches),0 ),(min_angle-max_angle))
          
            middle_sketch.sketchCurves.sketchLines.addByTwoPoints(arcOne.startSketchPoint, arcTwo.startSketchPoint)
            middle_sketch.sketchCurves.sketchLines.addByTwoPoints(arcOne.endSketchPoint, arcTwo.endSketchPoint)
        else:
            
            spacing = (max_angle - min_angle)/(roller_count-1)
            rotationMatrix = adsk.core.Matrix3D.create()
            startPoint = futil.pointFromOffset(futil.pZero,  -(diameter_inches/2 + ball_diameter_inches - compression_inches + roller_diameter/2/2.54), 0)
        ...
        second_comp.features.extrudeFeatures.addSimple(collection, adsk.core.ValueInput.createByReal(width - 2.54*0.0625), adsk.fusion.FeatureOperations.JoinFeatureOperation)


        pivotJoint = workingComp.asBuiltJoints.createInput(second_occ, occ, adsk.fusion.JointGeometry.createByPoint(pivotPoint))
        pivotJoint.setAsRevoluteJointMotion(adsk.fusion.JointDirections.ZAxisJointDirection)

        pivotjointMotion : adsk.fusion.RevoluteJointMotion = workingComp.asBuiltJoints.add(pivotJoint).jointMotion
        difference = max_angle - min_angle
        pivotjointMotion.rotationLimits.isMinimumValueEnabled = True
        pivotjointMotion.rotationLimits.isMaximumValueEnabled = True
        pivotjointMotion.rotationLimits.maximumValue = difference - min_angle
        pivotjointMotion.rotationLimits.minimumValue = - min_angle

        second_comp.isConstructionFolderLightBulbOn = False
        comp.isConstructionFolderLightBulbOn = False


        futil.get_random_flat_color(workingOcc, design, app)
        comp.name = f"Variable Hood Shooter"
